...
namespace fcm {
  ...
  void collide(
      ...
      glm::vec3 &force_1, glm::vec3 &force_2,
      glm::vec3 &torque_1, glm::vec3 &torque_2
  ) {
    (void) obj_1;
    (void) obj_2;
    (void) force_1;
  ...
  void collide(
    ...
    const float r1 = obj_1.radius;
    const float r2 = obj_2.radius;
    if (d > r1 + r2) {
      // no collision
      force_1 = {0, 0, 0};
      force_2 = {0, 0, 0};
    ...
    const glm::vec3 rad1 = glm::normalize(p2 - p1); // TODO this divides by 0 if p1 == p2!
    ...
    const float mu = static_friction(obj_1.mat, obj_2.mat);

    const glm::vec3 fforce_1 = mu * spring_mag * friction_dir(obj_1.velocity, force_1);
    const glm::vec3 fforce_2 = mu * spring_mag * friction_dir(obj_2.velocity, force_1);
